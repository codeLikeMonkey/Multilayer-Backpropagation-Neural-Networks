...
class Network:
    ...
    def __init__(self,layers,max_epoch,eta):
    ...
    def fit(self,images,lables):
        for i in range(self.max_epoch):
            self.train_with_mini_batch(images.copy(), lables.copy())
            print(self.check_accuracy(images.copy(),lables.copy()))


    def check_accuracy(self,images,lables):
        result = np.array([np.argmax(self.test(images[:,i])) for i in range(len(lables))])
        return np.sum(result == lables) / len(lables)
